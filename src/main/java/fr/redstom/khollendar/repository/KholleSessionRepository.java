...
public interface KholleSessionRepository
        extends CrudRepository<KholleSession, Long>,
                PagingAndSortingRepository<KholleSession, Long> {

    @Query(
            "SELECT ks FROM KholleSession ks JOIN ks.kholleSlots slot WHERE slot.dateTime > ?1"
