...
class KholleAssignmentServiceTest {
    ...
    @Mock private KholleSessionRepository sessionRepository;

    @Mock private UserPreferenceRepository preferenceRepository;

    @Mock private KholleAssignmentRepository assignmentRepository;

    @Mock private UserRepository userRepository;

    @InjectMocks private KholleAssignmentService assignmentService;
    ...
    void testAssignStudentsToSlots_Success() {
        ...
        when(assignmentRepository.saveAll(any()))
        ...
        for (Long count : slotDistribution.values()) {
            assertTrue(
                    count >= 1 && count <= 3,
                    "Chaque créneau devrait avoir entre 1 et 3 étudiants");
        }
    ...
    void testAssignStudentsToSlots_WithUnavailabilities_NeverAssignsToUnavailableSlot() {
        ...
        preferencesWithUnavailability.add(
                UserPreference.builder()
                        .id(100L)
                        .user(user1)
                        .session(session)
        ...
        verify(sessionRepository)
                .save(
                        argThat(
                                savedSession ->
                                        savedSession.status()
                                                == KholleSessionStatus.RESULTS_AVAILABLE));
    }
    ...
    void testGetAssignment() {
        ...
        KholleAssignment assignment =
                KholleAssignment.builder()
                        .id(1L)
                        .user(user)
                        .session(session)
                        .slot(slot)
        ...
        when(sessionRepository.findById(1L)).thenReturn(Optional.of(session));
        when(assignmentRepository.findByUserAndSession(user, session))
