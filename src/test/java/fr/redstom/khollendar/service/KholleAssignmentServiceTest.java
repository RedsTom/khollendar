...
class KholleAssignmentServiceTest {
    ...
    void testAssignStudentsToSlots_WithUnavailabilities_NeverAssignsToUnavailableSlot() {
        ...
        assertNotEquals(
                slots.get(0).id(), user1Assignment.id(), "User1 ne doit PAS être affecté au slot 1 (indisponible)");
        assertNotEquals(
                slots.get(1).id(), user1Assignment.id(), "User1 ne doit PAS être affecté au slot 2 (indisponible)");
        assertEquals(
                slots.get(2).id(), user1Assignment.id(), "User1 doit être affecté au slot 3 (seul créneau disponible)");
    }

    @Test
    void testAssignStudentsToSlots_MultipleUsersWithDifferentUnavailabilities() {
        // Given: Configuration complexe avec plusieurs utilisateurs ayant des indisponibilités
        // différentes
    ...
    void testAssignStudentsToSlots_UsersWithNoPreferences_AreAssignedRandomly() {
        ...
        for (int i = 3; i < 6; i++) {
            assertNotNull(assignments.get(users.get(i)), "User " + (i + 1) + " sans préférences doit être affecté");
        }
    }
    ...
    void testAssignStudentsToSlots_UnavailabilitiesRespectedEvenWhenNoCapacity() {
        ...
        assertEquals(slots.get(2).id(), user0Assignment.id(), "User0 doit être affecté au slot 3 (seul disponible)");
    }
    ...
    void testAssignStudentsToSlots_NoSlots() {
        // Given
        KholleSession emptySession =
                session.toBuilder().kholleSlots(Collections.emptyList()).build();
    ...
    void testAssignStudentsToSlots_UpdatesSessionStatus() {
        ...
        verify(sessionRepository)
                .save(argThat(savedSession -> savedSession.status() == KholleSessionStatus.RESULTS_AVAILABLE));
    }
    ...
    void testIsSessionAssigned_True() {
        // Given
        when(sessionRepository.findById(1L)).thenReturn(Optional.of(session));
        when(assignmentRepository.findBySession(session))
                .thenReturn(Arrays.asList(KholleAssignment.builder().build()));
    ...
    void testAssignStudentsToSlots_PreferencesAreBalanced() {
        ...
        // Then
        Map<Long, Long> slotCounts =
                assignments.values().stream().collect(Collectors.groupingBy(KholleSlot::id, Collectors.counting()));
